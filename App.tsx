...
const PlanCard: React.FC<{ plan: Plan }> = ({ plan }) => (
  <div className={`relative p-8 rounded-[2rem] border-2 transition-all duration-300 hover:scale-[1.02] ${plan.highlight ? 'bg-brand-dark border-brand-primary shadow-2xl shadow-brand-primary/20' : 'bg-white border-transparent hover:border-brand-primary/30 shadow-xl'}`}>
    {plan.highlight && <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-brand-primary text-brand-dark text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-wider shadow-lg">Mais Popular</div>}
    <h3 className={`text-2xl font-display font-bold mb-2 ${plan.highlight ? 'text-white' : 'text-brand-dark'}`}>{plan.name}</h3>
    <div className="flex items-baseline mb-6"><span className={`text-4xl font-bold ${plan.highlight ? 'text-brand-primary' : 'text-brand-dark'}`}>{plan.price}</span></div>
    <ul className="space-y-4 mb-8">{plan.features.map((feat, i) => (<li key={i} className="flex items-center gap-3"><div className={`w-5 h-5 rounded-full flex items-center justify-center ${plan.highlight ? 'bg-brand-primary/20 text-brand-primary' : 'bg-brand-light text-brand-dark'}`}><Check size={12} strokeWidth={3} /></div><span className={`text-sm font-medium ${plan.highlight ? 'text-slate-300' : 'text-slate-600'}`}>{feat}</span></li>))}</ul>
    <Button variant={plan.highlight ? 'primary' : 'secondary'} className={`w-full ${plan.highlight ? '' : 'border-slate-200 text-slate-600 hover:border-brand-primary hover:text-brand-dark'}`}>Escolher Plano</Button>
  </div>
);
...
function App() {
  ...
  return (
    <div className="min-h-screen font-sans bg-brand-light">
         ...
         <div className="max-w-7xl mx-auto px-4 grid md:grid-cols-2 gap-16 items-center">
            <div className="relative order-2 md:order-1">
               <div className="relative w-full aspect-square max-w-md mx-auto">
                  <div className="absolute inset-0 bg-gradient-brand rounded-full opacity-20 blur-3xl animate-pulse-slow"></div>
                  <div className="relative z-10 space-y-4">
                     {[1, 2, 3].map((i) => (
                       <div key={i} className={`glass-panel p-4 rounded-2xl flex items-center gap-4 transform transition-all hover:scale-105 duration-500 ${i === 2 ? 'translate-x-8' : ''}`}>
                          <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center"><Users size={20} className="text-brand-gray" /></div>
                          <div><div className="h-2 w-24 bg-slate-800/10 rounded mb-2"></div><div className="h-2 w-32 bg-slate-800/5 rounded"></div></div>
                       </div>
                     ))}
                  </div>
               </div>
      ...
      {/* --- 4. FEATURES --- */}
      <section id="uniques" className="py-24 bg-brand-light relative">
        <SectionTitle title="O Futuro do Entretenimento" subtitle="Esqueça o que você sabe sobre streaming. Aqui você é protagonista." />
        <div className="max-w-7xl mx-auto px-4 grid md:grid-cols-3 gap-6">
          {FEATURES.map((feature, idx) => (<div key={idx} className={idx === 0 || idx === 3 ? "md:col-span-2" : ""}><FeatureCard feature={feature} /></div>))}
        </div>
      </section>
      ...
      {/* --- 7. PLANS --- */}
      <section id="plans" className="py-24 bg-brand-light">
        <SectionTitle title="Escolha Seu Acesso" subtitle="De visitante a VIP. Tem espaço para todo mundo." />
        <div className="max-w-7xl mx-auto px-4 grid md:grid-cols-4 gap-6">{PLANS.map((plan, idx) => (<PlanCard key={idx} plan={plan} />))}</div>
      </section>
